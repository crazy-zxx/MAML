...
def nii2gz(nii_path):
    for fn in os.listdir(nii_path):
        fn = os.path.join(nii_path, fn)
        if os.path.isdir(fn):
            nii2gz(fn)
        elif os.path.splitext(fn)[-1] == '.nii':
            sitk.WriteImage(sitk.ReadImage(fn), os.path.splitext(fn)[0] + '.nii.gz')
            os.remove(fn)
            print('remove' + fn + '\nnew ' + fn + '.gz')
